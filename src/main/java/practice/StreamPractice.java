...
public class StreamPractice {
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        OptionalDouble average = IntStream.range(0, numbers.size())
                .map(s -> s % 2 == 1 ? numbers.get(s) - 1 : numbers.get(s))
                .filter(s -> s % 2 == 1)
                .average();
    ...
    public List<String> getCatsNames(List<Person> peopleList, int femaleAge) {
        Predicate<Person> pred = s -> s.getSex() == Person.Sex.WOMAN
                && s.getAge() >= femaleAge && s.getCats() != null;
    ...
    public List<String> validateCandidates(List<Candidate> candidates) {
        Predicate<Candidate> pred = new CandidateValidator();
